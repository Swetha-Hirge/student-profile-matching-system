...
export default function TeacherManager() {
  ...
  const submitCreate = async (e) => {
    ...
    const { username, email, subject, password } = form;
    if (!username || !email || !subject || !password) {
      return setErr('Please fill all fields.');
    }
    if (!/^\S+@\S+\.\S+$/.test(email)) {
  ...
  const removeTeacher = async (id) => {
    if (!Number.isInteger(Number(id))) return alert('Invalid teacher id');
    if (!window.confirm('Delete this teacher and their linked user account?')) return;
    try {
      await http.delete(`/api/teachers/${id}`);
      setRows(r => r.filter(x => x.id !== id));
    } catch (e) {
